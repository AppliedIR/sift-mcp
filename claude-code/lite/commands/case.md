...
**Script location:** `~/.vhir/bin/case-manager.sh`
...
1. Run: `bash ~/.vhir/bin/case-manager.sh status`
...
3. Run: `bash ~/.vhir/bin/case-manager.sh init <name> --description "<desc>"`
...
1. Run: `bash ~/.vhir/bin/case-manager.sh list`
2. Present the case list from the output

### `close [name]`

1. If no name, run: `bash ~/.vhir/bin/case-manager.sh close`
2. If name given, run: `bash ~/.vhir/bin/case-manager.sh close <name>`
3. Confirm closure to the user

## Important

- **NEVER write to `~/.vhir/active_case` directly.** The script does this.
