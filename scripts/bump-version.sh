...
# Update opensearch-mcp if present
if [[ -f "$OPENSEARCH_DIR/pyproject.toml" ]]; then
    sed -i "s/^version = \"[^\"]*\"/version = \"${VERSION}\"/" "$OPENSEARCH_DIR/pyproject.toml"
    echo "  Updated: ../opensearch-mcp/pyproject.toml"
    count=$((count + 1))
fi
