...
def find_registry_json_files(sources_dir: Path, custom_registry_dir: Path = None) -> Dict[str, List[Path]]:
    """
...
def navigate_to_key(root: Dict, key_path: str, _normalized_target: str = None) -> Optional[Dict]:
    """
...
def extract_autoruns(
    ...
    run_paths = [
        ...
        ("ROOT\\Microsoft\\Windows\\CurrentVersion\\RunServicesOnce", "RunServicesOnce"),
    ...
    if hive == 'SOFTWARE':
        ...
        run_paths.extend([
            ("ROOT\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options", "IFEO"),
        ])
        # Explorer extensions
        run_paths.extend([
            ("ROOT\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Browser Helper Objects", "BHO"),
            ("ROOT\\Microsoft\\Windows\\CurrentVersion\\Explorer\\ShellExecuteHooks", "ShellExecuteHooks"),
            ("ROOT\\Microsoft\\Windows\\CurrentVersion\\ShellServiceObjectDelayLoad", "ShellServiceObjectDelayLoad"),
        ])
        # Print Monitors
        run_paths.extend([
    ...
    if hive == 'NTUSER':
        ...
        run_paths.extend([
            ("ROOT\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run", "Run"),
            ("ROOT\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\RunOnce", "RunOnce"),
            ("ROOT\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Shell Folders", "ShellFolders"),
            ("ROOT\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Explorer\\User Shell Folders", "UserShellFolders"),
        ])
        ...
        for value_entry in values:
            ...
            try:
                ...
                if autorun_type == 'Winlogon':
                    ...
                    if value_name.lower() not in ['shell', 'userinit', 'taskman', 'system', 'vmapplet']:
                        continue
                    ...
                    if value_name.lower() not in ['authentication packages', 'notification packages', 'security packages']:
                        continue
...
def main():
    ...
    parser.add_argument("--registry-dir", type=str, help="Custom directory with extracted JSON files")
    ...
    if not all_files:
        ...
        logger.error("Clone: git clone https://github.com/AndrewRathbun/VanillaWindowsRegistryHives.git")
    ...
    try:
        for i, json_path in enumerate(all_files):
            ...
            logger.info(f"[{i+1}/{len(all_files)}] {os_info['short_name']} - {os_info['hive_type']}")
            ...
            if args.dry_run:
                ...
                logger.info(f"  Services: {stats['services']}, Tasks: {stats['tasks']}, Autoruns: {stats['autoruns']}")
