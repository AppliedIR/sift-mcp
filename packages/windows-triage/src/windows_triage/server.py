...
from .exceptions import ValidationError, DatabaseError, WindowsTriageError
...
class WindowsTriageServer:
    ...
    def __init__(
        ...
        known_good_path: Optional[Path] = None,
        ...
        if self.config.skip_db_validation:
            logger.warning("WT_SKIP_DB_VALIDATION is set — database validation disabled. Not recommended for production.")
        ...
        try:
            ...
            self.registry_db: Optional[RegistryDB] = None
            if reg_path.exists():
    ...
    def _validate_databases(self) -> None:
        ...
        try:
            ...
            logger.info(f"known_good.db: {kg_stats.get('files', 0)} files, "
                       f"{kg_stats.get('hashes', 0)} hashes")

            # Test context.db connectivity
            ctx_stats = self.context_db.get_stats()
            logger.info(f"context.db: {ctx_stats.get('lolbins', 0)} lolbins, "
    ...
    def _register_tools(self) -> None:
            ...
            return [
                Tool(
                    ...
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "path": {"type": "string", "description": "Windows file path"},
                            "hash": {"type": "string", "description": "Optional file hash (MD5/SHA1/SHA256)"},
                            "os_version": {"type": "string", "description": "Optional OS filter (e.g., 'Windows 10')"}
                        },
                        "required": ["path"]
                    }
                ),
                Tool(
                    ...
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "process_name": {"type": "string", "description": "Process name (e.g., 'svchost.exe')"},
                            "parent_name": {"type": "string", "description": "Parent process name"},
                            "path": {"type": "string", "description": "Optional executable path"},
                            "user": {"type": "string", "description": "Optional user context"}
                        },
                ...
                Tool(
                    ...
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "service_name": {"type": "string", "description": "Service name (e.g., 'BITS', 'Spooler')"},
                            "binary_path": {"type": "string", "description": "Service binary path - strongly recommended for hijacking detection"},
                            "os_version": {"type": "string", "description": "Target OS version (e.g., 'W11_22H2', 'W10_21H2', 'Server2022'). REQUIRED for accurate results."}
                        },
                ...
                Tool(
                    ...
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "task_path": {"type": "string", "description": "Task path (e.g., '\\Microsoft\\Windows\\UpdateOrchestrator\\Schedule Scan')"},
                            "os_version": {"type": "string", "description": "Target OS version (e.g., 'W11_22H2', 'W10_21H2'). REQUIRED for accurate results."}
                        },
                ...
                Tool(
                    ...
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "key_path": {"type": "string", "description": "Registry key path (e.g., 'SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run')"},
                ...
                Tool(
                    ...
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "key_path": {"type": "string", "description": "Registry key path (e.g., 'SOFTWARE\\Microsoft\\Windows\\CurrentVersion')"},
                            "value_name": {"type": "string", "description": "Optional: specific value name to check"},
                            "hive": {"type": "string", "description": "Optional: registry hive (SYSTEM, SOFTWARE, NTUSER, DEFAULT)"},
                            "os_version": {"type": "string", "description": "Optional: filter by OS version"}
                        },
                        "required": ["key_path"]
                    }
                ),
                ...
                Tool(
                    ...
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "filename": {"type": "string", "description": "Filename to analyze"}
                        },
                        "required": ["filename"]
                    }
                ),
                Tool(
                    ...
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "filename": {"type": "string", "description": "Filename to check (e.g., 'certutil.exe')"}
                        },
                        "required": ["filename"]
                    }
                ),
                Tool(
                    ...
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "dll_name": {"type": "string", "description": "DLL filename (e.g., 'version.dll')"}
                        },
                        "required": ["dll_name"]
                    }
                ),
                Tool(
                    ...
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "pipe_name": {"type": "string", "description": "Named pipe name"}
                        },
                        "required": ["pipe_name"]
                    }
                ),
        ...
        async def call_tool(name: str, arguments: dict):
            ...
            try:
                ...
                if name == "check_file":
                    _validate_input_length(arguments.get("path"), self.config.max_path_length, "path")
                    _validate_input_length(arguments.get("hash"), self.config.max_hash_length, "hash")
                    ...
                    _validate_input_length(arguments.get("process_name"), self.config.max_path_length, "process_name")
                    _validate_input_length(arguments.get("parent_name"), self.config.max_path_length, "parent_name")
                    _validate_input_length(arguments.get("path"), self.config.max_path_length, "path")
                    _validate_no_null_bytes(arguments.get("process_name"), "process_name")
                    ...
                    _validate_input_length(arguments.get("service_name"), self.config.max_service_name_length, "service_name")
                    _validate_input_length(arguments.get("binary_path"), self.config.max_path_length, "binary_path")
                    _validate_no_null_bytes(arguments.get("service_name"), "service_name")
                    ...
                    _validate_input_length(arguments.get("task_path"), self.config.max_task_path_length, "task_path")
                    ...
                    _validate_input_length(arguments.get("key_path"), self.config.max_key_path_length, "key_path")
                    ...
                    _validate_input_length(arguments.get("key_path"), self.config.max_key_path_length, "key_path")
                    _validate_input_length(arguments.get("value_name"), self.config.max_service_name_length, "value_name")
                    _validate_input_length(arguments.get("hive"), 20, "hive")
                    _validate_input_length(arguments.get("os_version"), self.config.max_service_name_length, "os_version")
                    _validate_no_null_bytes(arguments.get("key_path"), "key_path")
                    ...
                    _validate_input_length(arguments.get("filename"), self.config.max_path_length, "filename")
                    ...
                    _validate_input_length(arguments.get("dll_name"), self.config.max_path_length, "dll_name")
                    ...
                    _validate_input_length(arguments.get("pipe_name"), self.config.max_pipe_name_length, "pipe_name")
                ...
                error_result = {"error": "database_error", "message": "A database error occurred. Check server logs."}
                error_result = self._wrap_response(
                    name, arguments, error_result,
                    evidence_id=evidence_id, elapsed_ms=elapsed_ms,
                )
    ...
    async def _check_file(
        ...
        baseline_matches = self.known_good_db.lookup_by_path(normalized_path, os_version)
    ...
    async def _check_process_tree(
        ...
        _validate_input_length(process_name, self.config.max_path_length, "process_name")
        ...
        if suspicious_parents:
            parent_is_suspicious = parent_name.lower() in [p.lower() for p in suspicious_parents]
        ...
        if user:
            ...
            if user_type == 'SYSTEM':
                user_valid = user.upper() in ['SYSTEM', 'LOCAL SERVICE', 'NETWORK SERVICE', 'NT AUTHORITY\\SYSTEM']
            elif user_type == 'USER':
                user_valid = user.upper() not in ['SYSTEM', 'LOCAL SERVICE', 'NETWORK SERVICE']
            else:
                user_valid = True
    ...
    async def _check_service(
        ...
        # Defense in depth: validate inputs
        _validate_input_length(service_name, self.config.max_service_name_length, "service_name")
        _validate_input_length(binary_path, self.config.max_path_length, "binary_path")
        _validate_input_length(os_version, self.config.max_service_name_length, "os_version")
    ...
    async def _check_scheduled_task(
        ...
        if any(x in task_lower for x in ['\\temp\\', '\\tmp\\', '\\appdata\\', '\\public\\']):
            findings.append({
    ...
    async def _check_autorun(
        ...
        _validate_input_length(value_name, self.config.max_service_name_length, "value_name")
        _validate_input_length(os_version, self.config.max_service_name_length, "os_version")
        ...
        return {
            'verdict': 'UNKNOWN',
            'reasons': ['Autorun not in baseline (neutral - may be legitimate software)'],
            'confidence': 'low',
            'in_baseline': False
        }
    ...
    async def _check_registry(
        ...
        if value_name:
            ...
            matches = self.registry_db.lookup_value(key_path, value_name, hive, os_version)
        else:
        ...
        if in_baseline:
            ...
            result = {
                ...
                'confidence': 'high',
                'in_baseline': True,
                'os_versions': sorted(list(all_os_versions))[:10],  # Limit for readability
    ...
    async def _check_hash(self, hash_value: str) -> dict[str, Any]:
        ...
        is_vulnerable = False
        if algorithm:
            driver_info = self.context_db.check_vulnerable_driver(hash_normalized, algorithm)
            is_vulnerable = driver_info is not None

        # Calculate verdict
        verdict_result = calculate_hash_verdict(
            is_vulnerable_driver=is_vulnerable,
    ...
    async def _get_db_stats(self) -> dict[str, Any]:
        ...
        if self.registry_db and self.registry_db.is_available():
            ...
            result['registry_db'] = {'available': False, 'note': 'Optional 12GB database not installed'}
...
def main() -> None:
    ...
    logger.info(f"Starting windows-triage-mcp server")
