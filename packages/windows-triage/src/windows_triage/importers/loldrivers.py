...
def import_loldrivers(
    ...
    try:
        ...
        conn.commit()
        logger.info(f"Imported {stats['vulnerable_imported']} vulnerable drivers "
...
def _parse_loldriver_yaml(yaml_path: Path) -> Dict[str, Any] | None:
    ...
    for sample in data.get('KnownVulnerableSamples', []):
        ...
        samples.append({
            ...
            'sha256': sample.get('SHA256', '').lower() if sample.get('SHA256') else None,
            'sha1': sample.get('SHA1', '').lower() if sample.get('SHA1') else None,
            'md5': sample.get('MD5', '').lower() if sample.get('MD5') else None,
            'authentihash_sha256': authentihash.get('SHA256', '').lower() if authentihash.get('SHA256') else None,
            'authentihash_sha1': authentihash.get('SHA1', '').lower() if authentihash.get('SHA1') else None,
            'authentihash_md5': authentihash.get('MD5', '').lower() if authentihash.get('MD5') else None,
