...
class TestGithubHeaders:
    ...
    def test_falls_back_to_gh_cli(self, monkeypatch):
        ...
        with patch("windows_triage.scripts.download_databases.subprocess.run",
    ...
    def test_env_token_takes_priority(self, monkeypatch):
        ...
        with patch("windows_triage.scripts.download_databases.subprocess.run") as mock_run:
            headers = _github_headers()
...
class TestFetchRelease:
    ...
    def test_latest_succeeds_directly(self):
        ...
        with patch("windows_triage.scripts.download_databases.urllib.request.urlopen",
                    return_value=self._mock_urlopen(release)), \
             patch("windows_triage.scripts.download_databases._github_headers",
    ...
    def test_specific_tag(self):
        ...
        with patch("windows_triage.scripts.download_databases.urllib.request.urlopen",
                    return_value=self._mock_urlopen(release)) as mock_open, \
             patch("windows_triage.scripts.download_databases._github_headers",
                    return_value={"Accept": "application/vnd.github+json"}):
