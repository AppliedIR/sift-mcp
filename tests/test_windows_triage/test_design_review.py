...
class TestSHA256Support:
    ...
    def test_sha256_with_prefix(self):
        ...
        hash_with_prefix = "sha256:e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"
        assert detect_hash_algorithm(hash_with_prefix) == "sha256"
        assert normalize_hash(hash_with_prefix) == "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"

    def test_sha256_with_dash_prefix(self):
        """Test SHA-256 with dash prefix."""
        from windows_triage.analysis.hashes import detect_hash_algorithm, normalize_hash
        hash_with_prefix = "sha-256:e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"
        assert detect_hash_algorithm(hash_with_prefix) == "sha256"
...
class TestPipeChecking:
    ...
    def test_windows_pipe_detection(self, pipe_name, context_db_instance):
        ...
        assert result is not None, f"Expected {pipe_name} to be recognized as Windows pipe"
...
class TestHijackableDLL:
    ...
    def test_dll_case_insensitive(self, context_db_instance):
        ...
        assert (result1 == result2 == result3) or all(r in [None, []] for r in [result1, result2, result3])
...
class TestUnicodeEvasionExtended:
    ...
    def test_cyrillic_homoglyph_detection(self):
        ...
        for latin, cyrillic in self.CYRILLIC_HOMOGLYPHS.items():
            ...
            assert len(homoglyph_findings) > 0, f"Failed to detect Cyrillic {cyrillic} (looks like {latin})"
    ...
    def test_greek_homoglyph_detection(self):
        ...
        for latin, greek in self.GREEK_HOMOGLYPHS.items():
            ...
            findings = detect_unicode_evasion(text)
            homoglyph_findings = [f for f in findings if f['type'] == 'homoglyph']
            assert len(homoglyph_findings) > 0, f"Failed to detect Greek {greek} (looks like {latin})"
    ...
    def test_all_bidi_overrides(self):
        ...
        from windows_triage.analysis.unicode import detect_unicode_evasion, BIDI_OVERRIDES
    ...
    def test_all_zero_width_chars(self):
        ...
        from windows_triage.analysis.unicode import detect_unicode_evasion, ZERO_WIDTH_CHARS
    ...
    @pytest.mark.parametrize("leet_char,expected", [
        ('0', 'o'), ('1', 'i'), ('3', 'e'), ('4', 'a'),
        ('5', 's'), ('7', 't'), ('8', 'b'), ('@', 'a'),
        ('$', 's'), ('!', 'i'),
    ])
...
class TestFilenameAnalysisExtended:
    ...
    def test_space_padding_detection(self, filename):
        ...
        result = analyze_filename(filename)
        findings = result['findings']
        space_findings = [f for f in findings if f['type'] in ('space_padding', 'trailing_spaces')]
...
class TestVerdictCalculation:
    ...
    def test_critical_findings_suspicious(self):
        ...
        result = calculate_file_verdict(
            ...
            is_system_path=True,
            filename_findings=[{'type': 'hash_mismatch', 'severity': 'critical', 'description': 'test'}],
            lolbin_info=None
        )
...
class TestProcessExpectations:
    ...
    def test_expected_parent(self, process, parent, expected_valid, context_db_instance):
    ...
    def test_unexpected_parent(self, process, parent, expected_valid, context_db_instance):
...
class TestSuspiciousFilenames:
    ...
    def test_suspicious_filename_detection(self, filename, tool, category, context_db_instance):
        ...
        if result:  # May not be in DB if not imported
            assert result['tool_name'].lower() == tool.lower() or tool.lower() in result['tool_name'].lower()
    ...
    def test_case_insensitive_filename(self, context_db_instance):
        ...
        assert (result1 is None and result2 is None) or (result1 is not None and result2 is not None)
...
class TestExecutableExtensions:
    ...
    def test_double_extension_with_executable(self, ext):
        ...
        from windows_triage.analysis.filename import analyze_filename
        if ext in ['exe', 'scr', 'com', 'bat', 'cmd', 'ps1', 'vbs', 'js', 'hta', 'pif', 'msi']:
            filename = f"document.pdf.{ext}"
            result = analyze_filename(filename)
            double_ext = [f for f in result['findings'] if f['type'] == 'double_extension']
...
class TestVulnerableDrivers:
    ...
    def test_driver_hash_lookup_sha1(self, context_db_instance):
        ...
        db = context_db_instance
        result = db.check_vulnerable_driver("da39a3ee5e6b4b0d3255bfef95601890afd80709", "sha1")
        assert result is None or isinstance(result, dict)

    def test_driver_hash_lookup_sha256(self, context_db_instance):
        """Test vulnerable driver lookup by SHA256."""
        db = context_db_instance
        result = db.check_vulnerable_driver("e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855", "sha256")
        assert result is None or isinstance(result, dict)
    ...
    def test_driver_authentihash_md5(self, context_db_instance):
        ...
        db = context_db_instance
        result = db.check_vulnerable_driver(
            "authccdd1234567890abcdef12345678",
            "md5"
        )
        assert result is not None
        assert result.get('match_type') == 'authentihash'
...
class TestIntegration:
    ...
    def test_full_file_check_workflow(self, context_db_instance):
        ...
        from windows_triage.analysis.paths import normalize_path, extract_filename, is_system_path
        from windows_triage.analysis.filename import analyze_filename
    ...
    def test_malicious_file_detection_workflow(self, context_db_instance):
        ...
        assert any(f['type'] == 'double_extension' for f in filename_analysis['findings'])
...
class TestExtendedFuzzing:
    ...
    def test_random_hash_strings(self, _):
        ...
        from windows_triage.analysis.hashes import detect_hash_algorithm, validate_hash, normalize_hash
    ...
    def test_random_unicode_strings(self, _):
        ...
        from windows_triage.analysis.unicode import detect_unicode_evasion, normalize_homoglyphs
        from windows_triage.analysis.unicode import strip_invisible_chars, normalize_leet
        length = random.randint(1, 100)
        chars = []
        for _ in range(length):
    ...
    def test_random_paths(self, _):
        ...
        from windows_triage.analysis.paths import normalize_path, extract_filename
        from windows_triage.analysis.paths import extract_directory, is_system_path
...
class TestVerdictEdgeCases:
    ...
    def test_multiple_critical_findings(self):
        ...
        from windows_triage.analysis.verdicts import calculate_file_verdict
        findings = [
            {'type': 'hash_mismatch', 'severity': 'critical', 'description': 'mismatch'},
            {'type': 'double_extension', 'severity': 'critical', 'description': 'double ext'},
