...
class TestPathNormalizationExhaustive:
    ...
    SEPARATOR_CASES = [
        ...
        ("C:\\\\Windows\\\\cmd.exe", "\\\\windows\\\\cmd.exe"),  # Double backslashes preserved
...
class TestExtractDirectoryExhaustive:
    ...
    DIRECTORY_CASES = [
        ...
        ("cmd.exe", ""),
        ("", ""),
        ("C:\\Windows\\System32\\drivers\\etc\\hosts", "\\windows\\system32\\drivers\\etc"),
    ]
...
class TestUnicodeDetectionExhaustive:
    ...
    def test_cyrillic_homoglyph_detection(self, cyrillic, latin):
        ...
        assert len(homoglyph_findings) > 0, f"Should detect Cyrillic '{cyrillic}' looking like '{latin}'"
    ...
    def test_greek_homoglyph_detection(self, greek, latin):
        ...
        result = detect_unicode_evasion(test_name)
        homoglyph_findings = [f for f in result if f['type'] == 'homoglyph']
        assert len(homoglyph_findings) > 0, f"Should detect Greek '{greek}' looking like '{latin}'"
...
class TestTyposquattingExhaustive:
    ...
    PROTECTED_NAMES = [
        ...
        "services.exe", "winlogon.exe", "wininit.exe", "dwm.exe"
    ]
    ...
    def test_not_typosquatting(self, protected, not_typo):
        ...
        assert len(result) == 0, f"'{not_typo}' should not be flagged as typo of '{protected}'"
...
class TestFilenameAnalysisExhaustive:
    ...
    def test_double_extension_detection(self, filename, should_detect):
        ...
        if should_detect:
            assert len(de_findings) > 0, f"Should detect double extension in '{filename}'"
        else:
            assert len(de_findings) == 0, f"Should NOT detect double extension in '{filename}'"
    ...
    def test_short_name_detection(self, filename, should_detect):
        ...
        if should_detect:
            ...
            assert len(short_findings) == 0, f"Should NOT detect short name in '{filename}'"
    ...
    def test_space_padding_detection(self, filename, should_detect):
        ...
        from windows_triage.analysis.filename import analyze_filename
        result = analyze_filename(filename)
        space_findings = [f for f in result['findings']
                        if f['type'] in ('space_padding', 'trailing_spaces')]
        if should_detect:
            assert len(space_findings) > 0, f"Should detect space padding in '{filename!r}'"
        else:
            assert len(space_findings) == 0, f"Should NOT detect space padding in '{filename!r}'"
    ...
    def test_control_char_detection(self, filename, should_detect):
        ...
        if should_detect:
            assert len(ctrl_findings) > 0, f"Should detect control chars in '{filename!r}'"
        else:
            assert len(ctrl_findings) == 0, f"Should NOT detect control chars in '{filename!r}'"
...
class TestEntropyExhaustive:
    ...
    def test_low_entropy_names(self, filename):
        ...
        from windows_triage.analysis.filename import calculate_entropy, analyze_filename
        ...
        assert len(high_ent) == 0, f"'{filename}' should not be flagged for high entropy"
...
class TestVerdictCalculationExhaustive:
    ...
    def test_suspicious_critical_findings(self):
        ...
        from windows_triage.analysis.verdicts import calculate_file_verdict, Verdict
    ...
    def test_verdict_priority_suspicious_over_lolbin(self):
        ...
        result = calculate_file_verdict(
            ...
            filename_in_baseline=True,
            is_system_path=True,
            filename_findings=[{'type': 'homoglyph', 'severity': 'critical'}],  # Critical finding
            lolbin_info={'functions': ['Download', 'Execute']}  # Would be EXPECTED_LOLBIN
        )
        assert result.verdict == Verdict.SUSPICIOUS
...
class TestProcessVerdictExhaustive:
    ...
    def test_injection_detected_critical(self):
        ...
        from windows_triage.analysis.verdicts import calculate_process_verdict, Verdict
    ...
    def test_unknown_process_with_critical_finding(self):
        ...
        result = calculate_process_verdict(
            ...
            path_valid=None,
            user_valid=None,
            findings=[{'type': 'homoglyph', 'severity': 'critical', 'description': 'Unicode spoofing'}]
        )
        assert result.verdict == Verdict.SUSPICIOUS
        assert result.confidence == 'high'
...
class TestNLPStyleQuestions:
    ...
    def test_is_svchost_in_temp_suspicious(self):
        ...
        from windows_triage.analysis.paths import is_system_path, check_suspicious_path
    ...
    def test_is_word_spawning_cmd_suspicious(self):
        ...
        from windows_triage.importers.process_expectations import load_process_expectations
...
class TestProcessExpectationsExhaustive:
    ...
    def test_shells_have_suspicious_parents(self):
        ...
        for shell in shells:
            ...
            assert proc.get('suspicious_parents'), f"{shell} should have suspicious_parents"
            assert len(proc['suspicious_parents']) >= 50, f"{shell} should have many suspicious parents"

    def test_injection_targets_never_spawn(self):
        """Test injection targets have never_spawns_children=True."""
        from windows_triage.importers.process_expectations import load_process_expectations
        procs = load_process_expectations()
        injection_targets = ['lsass.exe', 'dwm.exe', 'audiodg.exe', 'fontdrvhost.exe', 'lsaiso.exe']
        for target in injection_targets:
            ...
            assert proc.get('never_spawns_children') is True, f"{target} should never spawn children"
...
class TestReconToolsExhaustive:
    ...
    RECON_TOOLS = [
        'whoami.exe', 'hostname.exe', 'ipconfig.exe', 'net.exe',
        'nltest.exe', 'dsquery.exe', 'systeminfo.exe', 'tasklist.exe',
        'netstat.exe', 'arp.exe', 'nslookup.exe', 'ping.exe',
        'tracert.exe', 'quser.exe', 'query.exe'
    ]
    ...
    def test_recon_tool_flags_office_parent(self, tool):
        ...
        proc = next((p for p in procs if p['process_name'] == tool), None)
        sp = proc.get('suspicious_parents', [])
        assert 'winword.exe' in sp or 'excel.exe' in sp, f"{tool} should flag Office as suspicious"
...
class TestAdditionalEdgeCases:
    ...
    def test_zero_width_between_every_char(self):
        ...
        from windows_triage.analysis.unicode import detect_unicode_evasion
        result = detect_unicode_evasion("s\u200Bv\u200Bc\u200Bh\u200Bo\u200Bs\u200Bt.exe")
...
class TestStressConditions:
    ...
    def test_very_deep_path(self):
        ...
        from windows_triage.analysis.paths import normalize_path, extract_filename
        depth = 100
        path = "C:\\" + "\\".join(["dir"] * depth) + "\\file.exe"
    ...
    def test_many_suspicious_parents(self):
        ...
        for proc in procs:
            ...
            assert len(sp) < 200, f"{proc['process_name']} has too many suspicious parents"
...
class TestServiceBinaryPathParsing:
    ...
    QUOTED_PATH_CASES = [
        ...
        ('"C:\\Program Files\\Service\\svc.exe" -arg1', "\\program files\\service\\svc.exe"),
        ('"C:\\Program Files\\Service\\svc.exe" -arg1 -arg2', "\\program files\\service\\svc.exe"),
    ...
    # Driver paths
    DRIVER_CASES = [
        ("\\SystemRoot\\System32\\drivers\\afd.sys", "\\windows\\system32\\drivers\\afd.sys"),
...
class TestLeetSpeakDetection:
    ...
    def test_leet_speak_detection(self, leet_name, expected_match):
        ...
        protected = ["svchost.exe", "lsass.exe", "csrss.exe", "winlogon.exe", "explorer.exe"]
...
class TestTyposquattingDetection:
    ...
    def test_typosquatting_detection(self, typo_name, expected_match):
        ...
        assert len(typo_findings) > 0, f"Should detect typosquatting: {typo_name} similar to {expected_match}"
